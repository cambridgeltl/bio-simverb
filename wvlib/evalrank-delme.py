...
def evaluate(wv, reference):
    ...
    def read_lexicon(antonym_file): 
            result = {}
            #print antonym_file
            with codecs.open(antonym_file, encoding='utf-8') as i:
                for i, line in enumerate(i):
                    word, antonym = line.strip("\n").strip().split()
        #                 word = words[0]
        #                 antonym= words[1]
                    result[word] =antonym
                    result[antonym] =word
            
            #print "lexicon length:" , len(result)
            return result
    #resultss = read_lexicon("/home/hwc25/python/gensim/retrofitting/lexicons/wn-antonyms.uniq_testing.txt")
    #resultsss = read_lexicon("/home/hwc25/python/gensim/retrofitting/lexicons/wn-synonyms.uniq_24pair.txt")
    for words, sim in sorted(reference, key=lambda ws: ws[1]):
        ...
        # if words[0] in resultss and words[1] == resultss[words[0]]:
        #     print "find", words[0], words[1]
        #     #result = cosine(v1, v2) / 1000
        #     result = -1.0 #cosine(v1, v2) / -1000.0
        # else:
        #     result = cosine(v1, v2)

        # if words[0] in resultsss and words[1] == resultsss[words[0]]:
        #     print "find", words[0], words[1]
        #     result = cosine(v1, v2) / 1000
        # else:
        #     result = cosine(v1, v2)
        ...
        result = cosine(v1, v2)
        predicted.append((words, result))
    if oov:
        info('OOV: ' + ', '.join(oov.keys()))
    simlist = lambda ws: [s for w,s in ws]
    rho, p = spearmanr(simlist(gold), simlist(predicted),use_ties=False)
