    ...
    def read_lexicon(antonym_file): 
            ...
            with codecs.open(antonym_file, encoding='utf-8') as i:
                for i, line in enumerate(i):
                    word, ID = line.strip("\n").strip().split()
                    result[word.strip()] =ID.strip()
            
            print "lexicon length:" , len(result)
            # for k, v in result.items():
            #     print k, v 
            return result
    resultss = read_lexicon("/home/hwc25/python/gensim/wvlib/word-similarities/bio-simverb/verb_ontology.txt")
    #resultss = read_lexicon("/home/hwc25/python/gensim/wvlib/word-similarities/bio-simverb/noun_ontology.txt")
    for words, sim in sorted(reference, key=lambda ws: ws[1]):
        w1, w2, v1, v2 = words[0], words[1], None, None
        try:
            v1 = wv[resultss[w1]]
            #print "find v1", resultss[w1],v1
        except KeyError:
            oov[w1] = True
        try:
            v2 = wv[resultss[w2]]
            #print "find v2", resultss[w2],v2
        except KeyError:
            oov[w2] = True
        if v1 is None or v2 is None:
            continue
        gold.append(((words,resultss[w1], resultss[w2]), sim))
        predicted.append(((words,resultss[w1], resultss[w2]), cosine(v1, v2)))
    if oov:
